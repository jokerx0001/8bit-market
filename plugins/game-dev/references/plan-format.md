...
| # | 行为 | 验证 |
|---|------|------|
| 1 | 玩家进入主菜单 → 看到角色选择界面 | 场景已切换到角色选择场景 |
| 2 | 玩家点击第2个角色卡片 → 卡片视觉高亮 | 第2个卡片的 selected 属性为 true |
| 3 | 玩家选中角色后点击"确认" → 跳转到游戏开始 | 当前场景为游戏场景 |
| 4 | 玩家未选中角色点击"确认" → 停留在当前界面 | 仍在角色选择场景，提示文本非空 |

**行为列表的作用：** test-agent 将它作为 testcase 的蓝图——每条行为对应一个 GUT testcase。coding-agent 将它作为实现目标——每条行为是必须支持的功能。集成测试据此设计玩家路径和视觉检查点。
...
5. **提取行为列表**：验证列描述 → 传给 test-agent 作为 GUT testcase 蓝图
...
- [ ] 验证列描述的是可被 GUT 断言的事实（场景切换、属性值、状态变化），不描述视觉状态（"看到"、"显示"）
- [ ] 中间产物已写入 `.work/` 子目录
- [ ] 禁止内容零命中（见下节）
