...
所有验证通过 `{test_runner} project test`。

| 测试文件 | 覆盖 |
|---------|------|
| {test_dir}/test_{name}.rpy | {交互行为简述}; visual: {视觉状态简述} |
```
...
| 测试文件 | 覆盖 |
|---------|------|
| {test_dir}/test_character_select.rpy | 角色选择交互（选中/取消/确认）; visual: 默认布局基线、选中高亮状态 |
```

**test agent 写测试时的信息来源：**
1. plan.md 测试策略 → 知道创建哪个文件、覆盖什么功能
2. `.work/design.md` → widget 树、变量名、交互流程（设计产物，不是代码）
3. 已有 `{test_dir}/test_*.rpy` → 代码风格和命名惯例
4. `references/renpy-testing.md` → Ren'Py 原生 testcase/testsuite API 完整参考
