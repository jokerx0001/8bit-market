...
| 类型 | 含义 | 包含 |
|------|------|------|
| `logic` | 行为/交互/数据逻辑——完成标准来自行为清单，不依赖 HTML 设计稿 | 界面骨架/组件树、流程跳转、数据初始化、持久化读写 |
| `ui` | 视觉还原——完成标准来自 HTML 设计稿 | 颜色/背景/字体/间距/状态样式——对照 HTML 精调视觉属性 |

`logic` 可以写界面骨架和组件树——不含精细视觉调整。`ui` 任务依赖对应的 logic 任务，在 logic 提供的骨架上做视觉还原。

**排序约束：** 所有 `logic` 任务排在 `ui` 任务前面。`ui` 任务依赖同界面的最后一个 `logic` 任务。
...
fix 模式下类型几乎总是 `logic`——极少涉及 visual 精调。回归测试囊括在第一个任务中：先锁定行为，再修复根因。
