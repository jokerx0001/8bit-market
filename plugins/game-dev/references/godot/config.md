...
非 headless 截图，用于 visual/ui 任务的视觉验证。

### 命令模板

```bash
# 截图脚本由 test-agent 在 RED 阶段编写，放在测试目录下
godot --path {project} --script {screenshot_script} | base64 -d > {output_path}.png
```

### 关键规则

- **绝不能加 `--headless`**（全黑 PNG）
- **切场景后必须 `await RenderingServer.frame_post_draw`**（等 GPU 提交）
- **Linux 无显示器用 `xvfb-run` 包裹**

完整方法、参考模式和已验证示例见 `${CLAUDE_PLUGIN_ROOT}/references/godot/screenshot.md`。
...
- **资源路径**: `**/*.tres`
- **测试路径**: `test/**/test_*.gd`
- **visual 截图脚本**: `test/visual/test_*.gd`
- **不修改**: `addons/`
