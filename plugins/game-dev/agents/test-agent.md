...
screenshot标识的行为,需要额外要求。这种行为需要额外增加一条截图testcase
screenshot标识行为必须有截图testcase,否则编写无效
...
同时将这条screenshot行为理解和整理，编写好要问visual-qa的问题内容, 写入同名 `.question` 文件：

```
{test_dir}/visual/test_{testcase_name}.question
```
这个文件的内容决定了visual-qa能否有效回答。visual-qa skill会读取截图和你给它的问题，会返回结论，告诉你截图是否符合你的描述。
