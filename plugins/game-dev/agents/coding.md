...
model: inherit
color: green
tools: ["Read", "Write", "Edit", "Glob", "Bash", "Grep", "WebFetch", "Skill"]
---
...
**资产查表：** design.md 中当前模块下的 `**资产:**` 声明块列出了需要的资产。去 resources.md 同名模块节找到对应条目，按策略执行：
- `code-only` → StyleBoxFlat / Theme / 代码绘制
- `CSG构造` → 读 3d-construction.md 构建
- `mmx` / `pillow` → 检查文件是否已生成，如果不存在则按规则5兜底
- `[HUMAN]` → 规则4 占位体
