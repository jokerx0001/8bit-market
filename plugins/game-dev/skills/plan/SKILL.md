...
解析用户的任务描述，生成需求文档。保存到 `{task_dir}/.work/requirements.md`：

** 需求设计定义 **
需求设计是指不涉及具体技术的、将用户描述转化为系统功能的设计。
同一份需求设计文档,由于不涉及具体技术,能选择任意能力足够的技术选型完成产品搭建。
比如需求设计完整描述了一个塔防游戏,那么技术选型可以是unity,godot,虚幻等任意框架,因为它们只需要选用对应的技术来实现需求设计,
就能得到一个完整的产品。
...
**确认行为清单（强制门）：** 从需求中提取**玩家可见的行为列表**，向用户确认：
...
**为什么这样做：** 确认行为非常重要，承担的是需求描述。设计文档描述的是方案（界面结构、组件树），行为描述的是需求（玩家看到什么、做什么、结果是什么）。方案可以有多种，行为只有一种。test-agent 和 coding-agent 都以行为为基准——测试断言行为，实现产出行。
...
**必须阅读文件**
- {task_dir}/.work/requirements.md 需求文档是用户需求的系统语言翻译,明确描述了系统应该是什么样子,充分阅读和理解才明白要做什么
...
**输入：** 步骤 5 的 `{task_dir}/.work/domain-design.md` + 步骤 6 的 `{task_dir}/.work/architecture.md`。

**拆分原则：按架构模块拆分，不按文件/行为清单拆分。**

architecture.md 的 "界面/场景划分" 表和数据流已经划定了模块边界——每个模块 = 一个 AI 任务候选。行为清单（来自步骤 4 的 requirements.md）的作用是确定每个模块的测试覆盖范围，不是用来直接生成任务。

**拆分流程：**

1. 读 `{task_dir}/.work/architecture.md` 的 "界面/场景划分" 表和数据流，提取模块列表
