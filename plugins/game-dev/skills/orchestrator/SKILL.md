...
**Grill 的目的是防止 AI 偏差，不是产出需求文档。** 通过 grilling（relentless interview）+ domain-modeling（术语 + ADR 归档）确保 AI 理解用户真正想要什么，避免自作主张跑偏。grilling 产出 interview 记录，domain-modeling 产出项目级的 CONTEXT.md 和 ADR
...
grilling 是核心采访引擎——逐题追问，一题一答，穷尽决策树。调用：
...
1. Skill({skill: "mattpocock-skills:grilling"}) 调用完成后，将其返回的文本内容原样写入 `grill-interview.md`
2. 检查文件：
   ```bash
...
grilling 完成后，调用 domain-modeling 将采访中确定下来的术语和架构决策结构化归档：

```
Skill({skill: "mattpocock-skills:domain-modeling"})
```

domain-modeling 自行维护项目级文件：
- `CONTEXT.md` — 术语表（glossary），跨 feat 共享
- `docs/adr/` — 架构决策记录，按需创建

**此步骤是补充性归档，不影响 grill-interview.md 的独立性。** domain-modeling 返回空或失败 → **不阻塞流程**，输出提示即可继续。
...
| `CONTEXT.md`、`docs/adr/` | domain-modeling 产出（项目级） | 术语定义 + 架构决策，跨 feat 共享 |
