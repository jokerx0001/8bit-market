...
**Grill 的目的是防止 AI 偏差，不是产出需求文档。** 重构场景下，用户对"想改成什么样"的描述常常不完整，通过 grilling（relentless interview）+ domain-modeling（术语 + ADR 归档）确保 AI 理解用户真正想要什么。
...
grilling 完成后，可调用 domain-modeling 将采访中确定下来的术语和架构决策结构化归档到项目级的 `CONTEXT.md` 和 `docs/adr/`：
...
| `{task_dir}/.work/grill-interview.md` | grilling 采访记录 | 理解用户确认过的重构目标，防止 AI 跑偏 |
