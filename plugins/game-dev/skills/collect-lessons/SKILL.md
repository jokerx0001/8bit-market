...
description: |
  ...
  区分测试和编码问题，查阅 Ren'Py 文档验证后写入 {dev_dir}/test-lessons.md
  和 {dev_dir}/coding-lessons.md，同时更新项目 CLAUDE.md。
...
检查 `{dev_dir}/current-state.json` 是否存在且有 `current_task`。如果不存在或从未有过任务 → 输出 "本次会话无 Ren'Py 开发活动，跳过经验收集" 后结束。
...
| 分类 | 归入文件 |
|------|---------|
| testcase/testsuite 语法、assert、click id、测试框架行为、测试配置、测试隔离 | `{dev_dir}/test-lessons.md` |
| Ren'Py screen/widget/样式/label/jump/call、框架行为（存档/回滚/persistent）、UI 原则 | `{dev_dir}/coding-lessons.md` |
