...
当追踪到代码引用了外部文件（`load()`、`preload()`、`Image.load_from_file()`、材质路径、纹理路径、GLB/PNG/SVG 等资源路径）时，必须检查文件是否存在：

```bash
test -f "{项目根目录}/{引用路径}" && echo "EXISTS" || echo "MISSING"
```

**找到一个缺失资产不停止。** 沿当前调用链继续往下追，检查该路径上所有引用的资产文件。一次追踪要把受影响的代码路径上所有缺失资产都找出来，写入 debug-analysis.md 的 `## 缺失资产` 章节。
...
| # | 检查项 | 状态 |
|---|--------|------|
| 0 | agent 是否在 debug-analysis.md 中写了"跳过"或"无需"？若是 → 本轮诊断无效，返回步骤 2 | ✅ / ❌ |
...
## 缺失资产
{仅当根因涉及缺失资产时填写。沿受影响的代码路径逐一检查后列出。此章节为空或不存在表示根因不涉及资产缺失。}

{逐条列出，格式如下：}
1. **{资产名称}** — 缺失文件: `{文件路径}`, 类型: {精灵/纹理/背景/模型/UI素材/材质}, 尺寸: {从上下文推断}, 用途: {在 BUG 场景中的作用}, 视觉要求: {从项目风格 + BUG 上下文推断}
2. ...

## 修复方向
{修复方案的概要描述，一两句话。不写实现代码。如果根因涉及缺失资产，修复方向应提及调用 art-resources-conductor 补充资产。}
```
