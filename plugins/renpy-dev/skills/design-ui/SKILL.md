...
从 `--task-dir` 参数解析 `task_dir`。如果未传入，从 `.renpy-dev/current-state.json` 读取 `current_task`。如果仍为空，报错退出。
...
扫描项目中的 Ren'Py GUI 配置和已有 screen 定义，提取风格数据：

**扫描范围：**

```bash
# GUI 配置文件
cat game/gui.rpy 2>/dev/null | head -200

# 已有 screen 定义
grep -rn "screen " game/ --include="*.rpy" | head -30

# 样式定义
grep -rn "style " game/ --include="*.rpy" | head -30
```
