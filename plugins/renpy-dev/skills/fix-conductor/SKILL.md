...
```
Skill({skill: "renpy-dev:artifact-manager", args: "create_task kind=fix phase=clarification"})
```

artifact-manager 会读取 `current-state.json`、递增 `counters.fix`、创建 `.renpy-dev/fix-{N}/`、写回状态。返回 `task_dir`。
