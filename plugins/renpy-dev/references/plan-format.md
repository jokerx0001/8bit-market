...
- [ ] 中间产物已写入 `.work/` 子目录
- [ ] **禁止内容零命中**：不含"lint 代替测试"、"人工验证"、"手动测试"、"测试缺失不阻塞"、"测试暂缓"、"跳过测试"、"Ren'Py Lint"等将非 `python tools/test.py` 手段作为验证替代方案的描述。验证手段唯一：`python tools/test.py`。测试基础设施缺失时 `[AI-0]` bootstrap 是强制的。
