...
```
- [AI-N] (type: {logic|ui}) {描述} → `{输出文件路径}` (依赖: AI-X, AI-Y)
- [AI-N] (type: ui, html: .work/layouts/{screen}.html) {描述} → `{输出文件路径}` (依赖: AI-X)
```
...
- `[AI-1]` (type: logic) 创建 CharacterSelectScreen 骨架 → `game/character_select.rpy`
- `[AI-2]` (type: logic) 创建角色选择跳转逻辑 → `game/script.rpy` (依赖: AI-1)
- `[AI-3]` (type: logic) 创建 CharacterSelect 测试 → `game/tests/test_character_select.rpy` (依赖: AI-1)
- `[AI-4]` (type: ui, html: .work/layouts/character_select.html) 角色选择界面视觉还原 → `game/character_select.rpy` (依赖: AI-3)
...
4. **提取输出路径**：匹配 `→ \`(.+)\`` 提取目标文件
5. **提取依赖**：匹配 `(依赖: (.+))` 确定执行顺序
6. **按依赖拓扑排序**：`logic` 任务优先于 `ui` 任务，同类型按依赖排序
7. **[HUMAN] 任务不执行**：最终汇总提醒用户
8. **解析测试策略表**：提取每条（测试文件、覆盖内容），与 `[AI-N]` 输出路径匹配
...
- [ ] 中间产物已写入 `.work/` 子目录
- [ ] **禁止内容零命中**：不含"lint 代替测试"、"人工验证"、"手动测试"、"测试缺失不阻塞"、"测试暂缓"、"跳过测试"、"Ren'Py Lint"等将非 `renpy.sh project test` 手段作为验证替代方案的描述。验证手段唯一：`renpy.sh project test`。game/tests/ 缺失时 `[AI-0]` bootstrap 是强制的。
