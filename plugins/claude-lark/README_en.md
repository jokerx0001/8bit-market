...
Two commands, never leave Claude Code:
...
`/setup-lark` guides you through entering App ID, App Secret, and phone number — auto-resolves Open ID, saves config, sends a test notification.

> If your team admin already gave you a config file, just place it at `~/.config/claude-lark/config.json` and skip `/setup-lark`.
